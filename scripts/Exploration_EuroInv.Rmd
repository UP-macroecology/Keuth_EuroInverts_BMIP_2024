...
In total data from 22 countries is available.
...
These countries have different amounts of study sites:
...
In general the whole data set ranges from 1968 to 2020, so 52 years. But not all years are covered in every country:
...
ggplot(data = completeness_plot, aes(x = country, y = Year)) +
  ...
  ggtitle("Coverage of sampled years")
...
ggplot(data = completeness.long, aes(x = country, y=proportion, fill = type))+
  ...
  labs(x = "", y = "Proportion") +
  theme(axis.text.x = element_text(hjust = 0.5))+
  ggtitle("Proportion of missing years")
```
