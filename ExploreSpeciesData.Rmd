...
Raya prepared the species data. Here we start to 
...
Wierd: the trend and standard error are zero. But this species is present on just one of the 202 "good" sites, so it's unlikely we would include it.
...
gp <- ggplot(data, aes(x=trend, y=logMCP2)) +
  geom_point(aes(colour = factor(p.value <= 0.05), size = log(nsite1))) +
